...
describe('<UserProvider />', () => {
  ...
  const userProviderMock: [any] = [{ user: {}, menus: [], errors: undefined, loading: true }];
  const props = {
    children: <Text />,
  };

  beforeEach(() => {
    jest.spyOn(UserProviderHooks, 'useUserProvider').mockReturnValue(userProviderMock);
  ...
  it('should render correctly when loading is false', () => {
    const mock: [any] = [{ ...userProviderMock[0], loading: false }];

    jest.spyOn(UserProviderHooks, 'useUserProvider').mockReturnValue(mock);
