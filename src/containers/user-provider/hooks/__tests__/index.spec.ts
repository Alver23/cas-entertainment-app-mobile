...
describe('UserProvider Hooks', () => {
  ...
  const fakeReducer: [any, jest.Mock] = [{ user: { id: 1, name: 'fake name' } }, jest.fn()];
  beforeEach(() => {
    jest.spyOn(React, 'useEffect').mockImplementation((callback) => callback());

    jest.spyOn(authHooks, 'useLocalSession').mockReturnValue({ token: fakeToken });

    jest.spyOn(React, 'useReducer').mockReturnValue(fakeReducer);

    jest.spyOn(userService, 'getMenus').mockResolvedValue([]);
  });
  ...
  it('should get a error when call getMenus method', () => {
    jest.spyOn(userService, 'getMenus').mockRejectedValue(new Error('fake error'));
    renderHook(() => useUserProvider());
    expect(fakeReducer[1]).toHaveBeenCalledTimes(2);
  });

  it('should call an one effect', () => {
    const reducer: [any, jest.Mock] = [{}, jest.fn()];
    jest.spyOn(React, 'useReducer').mockReturnValue(reducer);
